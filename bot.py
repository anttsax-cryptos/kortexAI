    ...
    if not is_greeting:
        if len(st.session_state.messages) > 1:
            rewriter_prompt = (
                ...
                "Respond STRICTLY with the keywords only, nothing else.\n\n"
        ...
        # 3. Live Web Search (ΠΛΗΡΗΣ ΟΔΗΓΙΑ ΓΙΑ ΠΡΟΪΟΝΤΑ & ΓΕΓΟΝΟΤΑ)
        with st.spinner(f"🔍 Searching the web for '{search_query}'..."):
            ...
            if results:
                search_context = (
                    ...
                    "Θέλω να γράψεις μια εξαιρετικά αναλυτική και πλήρη παρουσίαση, χωρίς να παραλείψεις καμία λεπτομέρεια από τα παραπάνω δεδομένα. "
                    "Ανάλογα με το θέμα της ερώτησης, οργάνωσε την απάντησή σου ΑΥΣΤΗΡΑ χρησιμοποιώντας έντονη γραφή (Bold) και Bullet Points ως εξής:\n\n"
                    "📦 ΑΝ ΠΡΟΚΕΙΤΑΙ ΓΙΑ ΠΡΟΪΟΝ (π.χ. τηλέφωνο, gadget, αυτοκίνητο, συσκευή):\n"
                    "- **Γενικές πληροφορίες** (με λίγα λόγια για το τι είναι)\n"
                    "- **Σχεδιασμός & Χαρακτηριστικά** (διαστάσεις, λειτουργίες, πλεονεκτήματα, μειονεκτήματα)\n"
                    "- **Τιμή** (κόστος και διαθεσιμότητα προϊόντος)\n\n"
                    "📢 ΑΝ ΠΡΟΚΕΙΤΑΙ ΓΙΑ ΓΕΓΟΝΟΣ / ΕΙΔΗΣΗ (π.χ. αγώνας, συναυλία, φυσικό φαινόμενο, είδηση):\n"
                    "- **Γενικό Πλαίσιο** (Πότε και πού συνέβη ή θα συμβεί, ποιοι εμπλέκονται)\n"
                    "- **Χρονικό & Λεπτομέρειες** (Αναλυτικά τι συνέβη, φάσεις του γεγονότος, σημαντικές στιγμές)\n"
                    "- **Αποτέλεσμα / Αντίκτυπος** (Σκορ, δηλώσεις, συνέπειες, επόμενα βήματα)\n\n"
